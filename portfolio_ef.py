...
def calculate_efficient_frontier(mean_returns, cov_matrix, num_points=100):
    ...
    For each target return, minimize portfolio variance subject to constraints
    ...
    for idx, target_return in enumerate(target_returns):
        try:
            ...
            try:
                ...
                try:
                    problem.solve(solver=cp.SCS, max_iters=2500)
                except:
                    # Last resort: OSQP
                    try:
            ...
            if w.value is not None and problem.status in ['optimal', 'optimal_inaccurate']:
                ...
                failed_count += 1
                if idx < 5 or idx >= num_points - 5:  # Print first/last few failures
                    print(f"  > Failed for target return {target_return*100:.2f}%: status={problem.status}")
        except Exception as e:
            failed_count += 1
            if idx < 5:  # Print first few exceptions
                print(f"  > Exception for target return {target_return*100:.2f}%: {e}")
