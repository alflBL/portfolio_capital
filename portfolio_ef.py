...
def calculate_efficient_frontier(mean_returns, cov_matrix, num_points=100):
    ...
    print(f"  > Attempting optimization-based efficient frontier...")
    ...
    for idx, target_return in enumerate(target_returns):
        try:
            ...
            # Constraints - RELAXED (no position size limits for EF calculation)
            constraints = [
                cp.sum(w) == 1,        # weights sum to 1
                w >= 0,                 # no short selling
                mu @ w >= target_return - 1e-6,  # Relaxed constraint
                mu @ w <= target_return + 1e-6   # Relaxed constraint
            ]
            
            # Solve with SCS (more robust for this)
            problem = cp.Problem(objective, constraints)
            
            try:
                problem.solve(solver=cp.SCS, eps=1e-4, max_iters=5000, verbose=False)
            except:
                try:
                    problem.solve(solver=cp.ECOS, abstol=1e-5, reltol=1e-5, max_iters=500, verbose=False)
                except:
                    try:
                        problem.solve(solver=cp.OSQP, eps_abs=1e-5, eps_rel=1e-5, max_iter=10000, verbose=False)
                    except:
                        pass
            ...
            if w.value is not None and problem.status in ['optimal', 'optimal_inaccurate']:
                ...
                if np.all(weights_val >= -1e-6) and np.abs(np.sum(weights_val) - 1.0) < 1e-3:
                    weights_val = np.maximum(weights_val, 0)  # Remove tiny negatives
                    weights_val = weights_val / np.sum(weights_val)  # Renormalize
    ...
    # If optimization got at least 50 points, use it
    if len(efficient_returns) >= 50:
        print(f"  > Using optimization-based efficient frontier!")
        ...
        print(f"  > WARNING: Optimization found only {len(efficient_returns)} portfolios. Falling back to Monte Carlo...")
        return calculate_efficient_frontier_monte_carlo(mean_returns, cov_matrix, num_portfolios=10000)

def calculate_efficient_frontier_monte_carlo(mean_returns, cov_matrix, num_portfolios=10000):
    """
    Fallback Monte Carlo method for efficient frontier
    Generates random portfolios and returns all of them (no bucketing)
    """
