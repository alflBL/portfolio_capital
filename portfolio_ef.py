...
def get_current_treasury_rate():
    ...
    try:
        ...
        # Try to get the most recent quote (intraday if market is open)
        info = treasury.info
        if 'regularMarketPrice' in info and info['regularMarketPrice'] is not None:
            rate = info['regularMarketPrice'] / 100
            print(f"   > Using LIVE 10-Year Treasury Rate: {rate*100:.2f}%")
            return rate
        else:
            # Fallback to most recent close
            rate = treasury.history(period="1d")['Close'].iloc[-1] / 100
            print(f"   > Using 10-Year Treasury Rate (latest close): {rate*100:.2f}%")
            return rate
    except:
...
def plot_efficient_frontier(name, tickers, mean_returns, cov_matrix, portfolios, returns_data, risk_free_rate):
    """Create Monte Carlo efficient frontier chart - clean arched display"""
    
    # Generate efficient frontier using Monte Carlo
    print(f"  > Generating 10,000 random portfolios for efficient frontier...")
    results = calculate_efficient_frontier(mean_returns, cov_matrix, num_portfolios=10000)
    ...
    # Scatter plot of all simulated portfolios (simple blue gradient)
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            size=3,
            color=returns_array,  # Color by return to create gradient
            colorscale='Blues',
            showscale=False,
            line=dict(width=0),
            opacity=0.6
        ),
        text=[f'Return: {r:.2%}<br>Risk: {v:.2%}' 
              for r, v in zip(returns_array, volatility_array)],
        hovertemplate='%{text}<extra></extra>',
        showlegend=False
    ))
    
    # Highlight Minimum Volatility portfolio (green circle - leftmost)
    fig.add_trace(go.Scatter(
    ...
    # Add current strategy portfolios
    ...
    for i, (strategy_name, portfolio) in enumerate(portfolios.items()):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                symbol='circle',  # All circles now
                line=dict(width=2, color='black')
            ),
            hovertemplate=f'{strategy_name}<br>Return: %{{y:.2%}}<br>Risk: %{{x:.2%}}<extra></extra>'
        ))
