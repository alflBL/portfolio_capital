...
def calculate_efficient_frontier_monte_carlo(mean_returns, cov_matrix, num_portfolios=1000):
    ...
    Generates random portfolios and extracts efficient frontier edge
    """
    ...
    for i in range(num_portfolios):
        ...
        all_risks.append(portfolio_std)
    ...
    # Extract efficient frontier: for each risk level, keep only the highest return
    # Sort by risk
    sorted_indices = np.argsort(all_risks)
    sorted_risks = all_risks[sorted_indices]
    sorted_returns = all_returns[sorted_indices]
    sorted_weights = all_weights[sorted_indices]
    
    # For each risk bucket, keep the max return
    efficient_returns = []
    efficient_risks = []
    efficient_weights = []
    
    # Divide into risk buckets
    num_buckets = 100
    min_risk = np.min(sorted_risks)
    max_risk = np.max(sorted_risks)
    risk_buckets = np.linspace(min_risk, max_risk, num_buckets)
    
    for i in range(len(risk_buckets) - 1):
        bucket_mask = (sorted_risks >= risk_buckets[i]) & (sorted_risks < risk_buckets[i+1])
        if np.any(bucket_mask):
            # Get portfolio with max return in this risk bucket
            bucket_returns = sorted_returns[bucket_mask]
            bucket_risks = sorted_risks[bucket_mask]
            bucket_weights = sorted_weights[bucket_mask]
            
            max_return_idx = np.argmax(bucket_returns)
            efficient_returns.append(bucket_returns[max_return_idx])
            efficient_risks.append(bucket_risks[max_return_idx])
            efficient_weights.append(bucket_weights[max_return_idx])
    ...
    # Pack into results format
    num_portfolios_efficient = len(efficient_returns)
    results = np.zeros((2 + num_assets, num_portfolios_efficient))
    results[0, :] = efficient_returns
    results[1, :] = efficient_risks
    
    for j in range(num_assets):
        results[2 + j, :] = efficient_weights[:, j]
    
    print(f"  > Monte Carlo complete: {num_portfolios_efficient} efficient portfolios extracted from {num_portfolios} simulations")
    return results
