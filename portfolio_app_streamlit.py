...
if st.session_state.analysis_started:
    try:
        ...
        if len(returns_array) == 0 or len(volatility_array) == 0:
            ...
            # Extract efficient frontier envelope (top edge of the cloud)
            # For each risk level, find the maximum return
            risk_bins = np.linspace(volatility_array.min(), volatility_array.max(), 100)
            envelope_risks = []
            envelope_returns = []
            
            for i in range(len(risk_bins) - 1):
                mask = (volatility_array >= risk_bins[i]) & (volatility_array < risk_bins[i+1])
                if np.any(mask):
                    max_return_idx = np.argmax(returns_array[mask])
                    bin_returns = returns_array[mask]
                    bin_risks = volatility_array[mask]
                    envelope_returns.append(bin_returns[max_return_idx])
                    envelope_risks.append(bin_risks[max_return_idx])
            
            envelope_returns = np.array(envelope_returns)
            envelope_risks = np.array(envelope_risks)
            
            # Plot efficient frontier envelope (smooth line)
            fig.add_trace(go.Scatter(
                x=envelope_risks,
                y=envelope_returns,
                mode='lines',
