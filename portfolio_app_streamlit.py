...
if st.session_state.analysis_started:
    try:
        ...
        if len(returns_array) == 0 or len(volatility_array) == 0:
            ...
            # Extract truly efficient portfolios from Monte Carlo results
            # Sort by volatility
            sorted_indices = np.argsort(volatility_array)
            sorted_vols = volatility_array[sorted_indices]
            sorted_rets = returns_array[sorted_indices]
            
            # Keep only points where return is increasing (efficient frontier edge)
            efficient_vols = [sorted_vols[0]]
            efficient_rets = [sorted_rets[0]]
            
            for i in range(1, len(sorted_vols)):
                # Only add if return is higher than previous max
                if sorted_rets[i] > efficient_rets[-1]:
                    efficient_vols.append(sorted_vols[i])
                    efficient_rets.append(sorted_rets[i])
            
            efficient_vols = np.array(efficient_vols)
            efficient_rets = np.array(efficient_rets)
            
            # Smooth the curve using scipy spline
            from scipy.interpolate import UnivariateSpline
            
            if len(efficient_vols) >= 4:
                # Create smooth spline through efficient points
                spline = UnivariateSpline(efficient_vols, efficient_rets, s=0.0001, k=3)
                
                # Generate smooth curve
                smooth_risks = np.linspace(efficient_vols.min(), efficient_vols.max(), 300)
