...
if st.session_state.analysis_started:
    try:
        ...
        results = calculate_efficient_frontier(your_mean_returns, your_cov_matrix, num_points=100)
        
        # Extract returns and volatility
        returns_array = results[0, :]
        volatility_array = results[1, :]
        
        print(f"DEBUG: Efficient Frontier Data")
        print(f"  Returns array shape: {returns_array.shape}")
        print(f"  Volatility array shape: {volatility_array.shape}")
        print(f"  Number of portfolios: {len(returns_array)}")
        print(f"  Return range: {returns_array.min()*100:.2f}% to {returns_array.max()*100:.2f}%")
        print(f"  Risk range: {volatility_array.min()*100:.2f}% to {volatility_array.max()*100:.2f}%")
        
        # Check if we have valid results
        if len(returns_array) == 0 or len(volatility_array) == 0:
            st.error("Unable to calculate efficient frontier. This may be due to incompatible portfolio constraints or insufficient data.")
            st.info("Try adjusting your tickers, time period, or position size constraints.")
        else:
            ...
            max_sharpe_ratio_val = sharpe_ratios[max_sharpe_idx]
            ...
            cml_y = risk_free_rate + max_sharpe_ratio_val * cml_x
            ...
            # Plot efficient frontier (visible blue dots - bright and clear)
            fig.add_trace(go.Scatter(
                x=volatility_array,
                y=returns_array,
                mode='markers',
                ...
                marker=dict(
                    size=4,
                    color='rgba(100, 150, 255, 0.4)',  # Bright blue, semi-transparent
                    line=dict(width=0)
                ),
            ...
            # Create smooth efficient frontier from strategy portfolios
            # Collect strategy points (these are already optimized)
            strategy_points = []
            for strategy_name, portfolio in portfolios.items():
            ...
            # Sort by risk
            strategy_points.sort(key=lambda x: x['risk'])
            
            # Extract sorted risks and returns
            ef_risks = np.array([p['risk'] for p in strategy_points])
            ef_returns = np.array([p['return'] for p in strategy_points])
            
            # Interpolate to create smooth curve
            from scipy.interpolate import UnivariateSpline
            
            # Create spline interpolation (smooth curve through strategy points)
            if len(ef_risks) >= 3:  # Need at least 3 points for spline
                # Sort to ensure monotonic x
                sort_idx = np.argsort(ef_risks)
                ef_risks_sorted = ef_risks[sort_idx]
                ef_returns_sorted = ef_returns[sort_idx]
                
                # Remove duplicates
                unique_mask = np.concatenate(([True], np.diff(ef_risks_sorted) > 1e-6))
                ef_risks_unique = ef_risks_sorted[unique_mask]
                ef_returns_unique = ef_returns_sorted[unique_mask]
                
                if len(ef_risks_unique) >= 3:
                    # Create smooth spline
                    spline = UnivariateSpline(ef_risks_unique, ef_returns_unique, s=0, k=min(3, len(ef_risks_unique)-1))
                    
                    # Generate smooth curve
                    smooth_risks = np.linspace(ef_risks_unique.min(), ef_risks_unique.max(), 200)
                    smooth_returns = spline(smooth_risks)
                    
                    # Plot smooth efficient frontier
                    fig.add_trace(go.Scatter(
                        x=smooth_risks,
                        y=smooth_returns,
                        mode='lines',
                        name='Efficient Frontier Edge',
                        line=dict(color='darkblue', width=3),
                        hovertemplate='Efficient Frontier<br>Risk: %{x:.2%}<br>Return: %{y:.2%}<extra></extra>',
                        showlegend=True
                    ))
                else:
                    # Fallback: just draw line through points
                    fig.add_trace(go.Scatter(
                        x=ef_risks_unique,
                        y=ef_returns_unique,
                        mode='lines',
            ...
            # Add tangency line (from risk-free rate to Max Sharpe point)
            tangency_x = [0, max_sharpe_risk]
            tangency_y = [risk_free_rate, max_sharpe_return]
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate='Tangency Line<br>Risk: %{x:.2%}<br>Return: %{y:.2%}<extra></extra>',
                showlegend=True
            ))
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate=f'Max Sharpe Portfolio<br>Return: {max_sharpe_return:.2%}<br>Risk: {max_sharpe_risk:.2%}<br>Sharpe: {max_sharpe_ratio_val:.3f}<extra></extra>'
            ))
            ...
            # Max Sharpe shown as tangency point, others shown separately
            strategy_colors = {
                'Max Sharpe': 'red',
                'Min Variance': 'orange', 
                'Equal Weight': 'purple',
            ...
            for strategy_name, portfolio in portfolios.items():
                # Skip only Max Sharpe - already shown as tangency
        ...
        if analyze_prospect and prospect_tickers_input:
            ...
            try:
                ...
                # Generate efficient frontier using optimization
                prospect_results = calculate_efficient_frontier(prospect_mean_returns, prospect_cov_matrix, num_points=100)
                
                # Extract returns and volatility
                prospect_returns_array = prospect_results[0, :]
                prospect_volatility_array = prospect_results[1, :]
                
                # Calculate Sharpe ratios
                prospect_sharpe_ratios = (prospect_returns_array - risk_free_rate) / prospect_volatility_array
                
                # Find max Sharpe ratio portfolio
                prospect_max_sharpe_idx = np.argmax(prospect_sharpe_ratios)
                prospect_max_sharpe_return = prospect_returns_array[prospect_max_sharpe_idx]
                prospect_max_sharpe_risk = prospect_volatility_array[prospect_max_sharpe_idx]
                prospect_max_sharpe_ratio_val = prospect_sharpe_ratios[prospect_max_sharpe_idx]
                
                # Find min volatility portfolio
                prospect_min_vol_idx = np.argmin(prospect_volatility_array)
                
                # Capital Market Line
                prospect_cml_x = np.linspace(0, prospect_max_sharpe_risk * 1.5, 100)
                prospect_cml_y = risk_free_rate + prospect_max_sharpe_ratio_val * prospect_cml_x
                
                # Create plot
                prospect_fig = go.Figure()
                
                # Plot Capital Market Line
                prospect_fig.add_trace(go.Scatter(
                    x=prospect_cml_x,
                    y=prospect_cml_y,
                    mode='lines',
                    name='Capital Market Line (CML)',
                ...
                # Plot efficient frontier (dots only, no line)
                prospect_fig.add_trace(go.Scatter(
                    x=prospect_volatility_array,
                    y=prospect_returns_array,
                ...
                # Highlight Max Sharpe portfolio
                prospect_fig.add_trace(go.Scatter(
                    x=[prospect_max_sharpe_risk],
                    y=[prospect_max_sharpe_return],
                    mode='markers',
                    name='Max Sharpe (Tangency)',
                    marker=dict(
                        size=20,
                    ...
                    hovertemplate=f'Max Sharpe Portfolio<br>Return: {prospect_max_sharpe_return:.2%}<br>Risk: {prospect_max_sharpe_risk:.2%}<br>Sharpe: {prospect_max_sharpe_ratio_val:.3f}<extra></extra>'
                ))
                
                # Highlight Minimum Volatility portfolio
                prospect_fig.add_trace(go.Scatter(
                    x=[prospect_volatility_array[prospect_min_vol_idx]],
                    y=[prospect_returns_array[prospect_min_vol_idx]],
                    mode='markers',
                    name='Min Volatility',
                    marker=dict(
                        size=18,
                    ...
                    hovertemplate=f'Min Volatility<br>Return: {prospect_returns_array[prospect_min_vol_idx]:.2%}<br>Risk: {prospect_volatility_array[prospect_min_vol_idx]:.2%}<extra></extra>'
                ))
                
                # Add strategy portfolios (larger, fully opaque)
                for strategy_name, portfolio in prospect_portfolios.items():
                    # Skip only Max Sharpe - already shown as tangency
                    if strategy_name == 'Max Sharpe':
                        continue
                ...
                prospect_fig.update_xaxes(tickformat='.1%')
                prospect_fig.update_yaxes(tickformat='.1%')
                
                st.plotly_chart(prospect_fig, use_container_width=True)
